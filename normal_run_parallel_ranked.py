...
def produce_features(result, n_features,methods):
    ...
    for i in range(len(methods)):
        ...
        df.to_csv(results_path+method+'.csv')

def produce_features_sfs(result, n_features,methods):
    n_runs = 2
    for i in range(len(methods)):
        arr = [0 for i in range(n_features)]
        method = methods[i]
...
def execute_feature_selection(runs, methods):
    ...
    produce_features(result, 5, methods)
    return result
...
def execute_feature_selection_sfs(runs,classifiers, metric):
    ...
    produce_features_sfs(result,5,methods)
    return result
